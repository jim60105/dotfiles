...
install_chezmoi() {
  ...
  echo "Init submodules..." >&2
  if ! git submodule update --init --recursive; then
    echo "Warning: Failed to update git submodules" >&2
  fi

  # Fetch latest changes for all submodules
  echo "Fetching latest changes for submodules..." >&2
  if ! git submodule foreach --recursive git fetch; then
    echo "Warning: Failed to fetch submodule changes" >&2
  fi

  # Checkout master branch for all submodules
  echo "Checking out master branch for submodules..." >&2
  if ! git submodule foreach --recursive 'git checkout origin/master'; then
    echo "Warning: Failed to checkout master branch for submodules" >&2
  fi
